...
void init_PB(void)
{
    RCC_AHB1ENR |= 0x01 << 2;                 // Activation du GPIOC
    GPIOC_MODER = (GPIOC_MODER & 0xF3FFFFFF); // On mets GPIOC_MODER à 01 : General purpose input
    GPIOC_PUPDR |= 0xF7FFFFFF;
}
...
void init_USART(void)
{
    ...
    USART2_CR3 = 0;
    USART2_CR2 = 0;
    USART2_CR1 = (1 << 2) | (1 << 3) | (1 << 13) | (1 << 5);
    nvic_enable_irq(38);
    nvic_set_priority(38, 1);
...
int counter;
int frequency;
const char *current_string = NULL;
...
void usart2_isr(void)
{
    ...
    {
        frequency -= 2000;
        systick_set_frequency(frequency, get_SYSCLK());
        c = 'p';
    }

    if ((USART2_SR & (1 << 7)) != 0)
    {
        if (current_string[string_index] != '\0')
        {
            USART2_DR = current_string[string_index++];
        }
        else
        {
            current_string = NULL;
