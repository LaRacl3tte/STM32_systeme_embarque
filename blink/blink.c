...
static void gpio_setup(void) {
    ...
    /* Manually: */
    GPIOA_MODER =
        (GPIOA_MODER & ~GPIO_MODE_MASK(5)) | GPIO_MODE(5, GPIO_MODE_OUTPUT);
    GPIOA_OTYPER = (GPIOA_OTYPER & ~GPIO5) | (GPIO_OTYPE_PP << 5);
    GPIOA_OSPEEDR = (GPIOA_OSPEEDR & ~GPIO_OSPEED_MASK(5)) |
                    GPIO_OSPEED(5, GPIO_OSPEED_100MHZ);
    GPIOA_PUPDR =
        (GPIOA_PUPDR & ~GPIO_PUPD_MASK(5)) | GPIO_PUPD(5, GPIO_PUPD_NONE);
#else
    /* Using API functions: */
